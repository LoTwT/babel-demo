...
function resolveType(targetType, referenceTypesMap = {}, scope) {
  ...
  switch (targetType.type) {
    ...
    case "TSTypeReference":
      ...
      const paramsTypes = targetType.typeParameters.params.map((item) =>
        resolveType(item),
      )
      const params = typeAlias.paramsNames.reduce((obj, name, index) => {
        obk[name] = paramTypes[index]
        return obj
      }, {})
    case "TSLiteralType":
    ...
    visitor: {
      ...
      CallExpression(path, state) {
        ...
        functionDeclarePath.node.typeParameters.params.map(
          (item, index) => (realTypeMap[item.name] = realTypes[index]),
        )
